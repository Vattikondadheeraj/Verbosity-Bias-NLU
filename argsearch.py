...
class ARGS:
    ...
    def generate_step(self, mout, input_ids, pre_screen_beam_width=40, weight=0., method="greedy", temperature=0.7, rm_cached=None, debug=True):
        ...
        if rm_cached is None:
            #print(**self.LLM.prepare_inputs_for_generation(input_ids=flat_trme.to(self.rm_dev)))
            llm_inputs=self.LLM.prepare_inputs_for_generation(input_ids=flat_trme.to(self.rm_dev), attention_mask=create_attention_mask(flat_trme.shape[1], flat_trme.shape[0]).to(self.rm_dev), past_key_values=None, use_cache=True)
            rm_inputs = {k: v for k, v in llm_inputs.items() if k in ['input_ids', 'past_key_values', 'use_cache', 'attention_mask']}
            rm_out = self.RM(**rm_inputs)
            rm_cached = rm_out.past_key_values
        else:
            llm_inputs=self.LLM.prepare_inputs_for_generation(input_ids=flat_trme.to(self.rm_dev), attention_mask=create_attention_mask(flat_trme.shape[1], flat_trme.shape[0]).to(self.rm_dev), past_key_values=rm_cached, use_cache=True)
            #llm_inputs=self.LLM.prepare_inputs_for_generation(input_ids=flat_trme.to(self.rm_dev), attention_mask=create_attention_mask(flat_trme.shape[1], flat_trme.shape[0]).to(self.rm_dev), past_key_values=None, use_cache=True)
            rm_inputs = {k: v for k, v in llm_inputs.items() if k in ['input_ids', 'past_key_values', 'use_cache', 'attention_mask']}
            rm_out = self.RM(**rm_inputs)
            rm_cached = rm_out.past_key_values
    ...
    def generate_lookahead_step(self, mout, input_ids, pre_screen_beam_width=10, weight=0., rm_cached=None, debug=True, method="greedy",lookahead=2):
        # Similar to your existing generate_step logic, but after selecting candidate tokens,
        # you perform lookahead evaluation for each and adjust the selection based on lookahead rewards.
        out_logits = mout.logits[:, -1]
        prescreen_logits, prescreen_tokens = torch.topk(out_logits, dim=-1, k=pre_screen_beam_width)
        lookahead_rewards = []
        
        for token_idx in range(pre_screen_beam_width):
            candidate_token = prescreen_tokens[:, token_idx:token_idx+1]  # Get the token
            candidate_input = torch.cat((input_ids, candidate_token), dim=1)
            # Perform lookahead evaluation for each candidate to get rewards
            lookahead_reward = self.lookahead_evaluation(candidate_input, rm_cached,lookahead)
            lookahead_rewards.append(lookahead_reward)
        prev_best_token_idx=torch.argmax(prescreen_logits.flatten())
        prev_best_token=prescreen_tokens[:, prev_best_token_idx].unsqueeze(-1)
        
        lookahead_rewards = torch.tensor(lookahead_rewards).to(input_ids.device)
        # Selecting the token with the best lookahead reward
        new_scores=weight*lookahead_rewards.flatten() + prescreen_logits.flatten()
        best_token_idx = torch.argmax(new_scores)
        best_token = prescreen_tokens[:, best_token_idx].unsqueeze(-1)
        
        # Prepare for the next generation step
        input_ids = torch.cat((input_ids, best_token), dim=1)
        # Update `rm_cached` and other necessary states here
        # Note: The actual integration and updating of caches/states may need to be adapted based on your logic
        
        return input_ids, rm_cached, prev_best_token, best_token, prescreen_logits.flatten(), new_scores, lookahead_rewards.flatten()

    def lookahead_evaluation(self, initial_tokens, initial_cached, lookahead=2):
        """
        Generate tokens for 'depth' steps ahead for a given initial token and cache,
        and evaluate the sequence using RM to get a reward.
        """
        tokens = initial_tokens
        cached = initial_cached
        for _ in range(lookahead):
            with torch.no_grad():
                mout = self.LLM(**self.LLM.prepare_inputs_for_generation(
                    input_ids=tokens,
                ...
                next_token_logits = mout.logits[:, -1, :]  # Get the logits for the last token position
                next_token = torch.argmax(next_token_logits, dim=-1, keepdim=True)
                tokens = torch.cat((tokens, next_token), dim=1)
        
        # Evaluate the final sequence with RM
        llm_inputs=self.LLM.prepare_inputs_for_generation(input_ids=tokens.to(self.rm_dev),attention_mask=create_attention_mask(tokens.shape[1], tokens.shape[0]).to(self.rm_dev),use_cache=False)        
        rm_inputs = {k: v for k, v in llm_inputs.items() if k in ['input_ids', 'past_key_values', 'use_cache', 'attention_mask']}
        rm_out = self.RM(**rm_inputs)
        reward = rm_out.logits.squeeze().item()  # Simplified reward extraction
        
        return reward
    
    def generate(self, prompt, weight=0., topk=1, max_new_token=180, method="greedy", temperature=0.7, chunk_size=5, debug=False,lookahead=2):
        ...
        for _ in iterator_obj:
            ...
            with torch.no_grad():
                ...
                if method == "greedy_large":
                    ...
                    tokens, rm_cached, prev_best_token, best_token, prescreen_logit, new_score, lookahead_rewards  = self.generate_lookahead_step(mout, tokens, topk, weight, rm_cached, debug, method,lookahead)
                    prev_best_tokens.append(prev_best_token)
                    best_tokens.append(best_token)
                    prescreen_logits.append(prescreen_logit)
                    new_scores.append(new_score)
                    rewards.append(lookahead_rewards)
                del mout

        return tokens, prev_best_tokens, best_tokens, prescreen_logits, new_scores, rewards
